...
void CostmapLayer::updateWithMax(layered_costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
...
void CostmapLayer::updateWithTrueOverwrite(layered_costmap_2d::Costmap2D& master_grid, int min_i, int min_j,
                                           int max_i, int max_j)
{
  if (!enabled_)
    return;
...
void CostmapLayer::updateWithOverwrite(layered_costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
  unsigned char* master = master_grid.getCharMap();
...
void CostmapLayer::updateWithAddition(layered_costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
  unsigned char* master_array = master_grid.getCharMap();
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    for (int i = min_i; i < max_i; i++)
    {
      ...
      else
      {
        ...
        if (sum >= layered_costmap_2d::INSCRIBED_INFLATED_OBSTACLE)
            master_array[it] = layered_costmap_2d::INSCRIBED_INFLATED_OBSTACLE - 1;
        else
            master_array[it] = sum;
      }
      it++;
